...
// Path
const routePath = "/api/user-accounts";
...
Deno.test(`POST ${routePath}`, async () => {
    ...
    if (mongoose && isConnected) {
        ...
        try {
            ...
            assertEquals(res.body.type, dataNew.type);
            assertEquals(res.body.purchase, dataNew.purchase);
            assertEquals(new Date(res.body.expiresAt), new Date(dataNew.expiresAt));
...
Deno.test(`GET ${routePath}`, async () => {
    ...
    if (mongoose && isConnected) {
        ...
        try {
            ...
            assertEquals(typeof res.body[0].purchase, "number");
        } catch(error) {
            console.log('ERROR: ', error)
        }
...
Deno.test(`GET ${routePath}/:id`, async () => {
    ...
    if (mongoose && isConnected) {
        ...
        try {
            ...
            assertEquals(typeof res.body.purchase, "number");
...
Deno.test(`PUT ${routePath}/:id`, async () => {
    ...
    if (mongoose && isConnected) {
        ...
        try {
            ...
            assertEquals(res.status, 200);
            assertEquals(res.body._id, testModelId);
            assertEquals(res.body.type, dataUpdate.type);
        } catch(error) {
            console.log('ERROR: ', error);
        }
